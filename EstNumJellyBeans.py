...
class NumJellyEstimator:
    ...
    def set_world_pop(self, people):
        ...
        assert type(people) is int, 'Number of people must correspond to an integer'
 
        # NE24: Add a test for value here
        if people > 1e11:
            print "Whoa there, that's a bit too many people"
            sys.exit()
    ...
    def set_frac_ppl_loving_pink(self, frac):
        ...
        # NE24: Add a test for type here
        assert type(frac) is float, "Sorry, fraction must be a float"

        # NE24: Add a test for value here
        if frac <= 0.0 or frac >= 1.0:
            print "Oh no! The fraction must be between 0 and 1!" 
